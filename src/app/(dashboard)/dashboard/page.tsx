...
// ── Palette constants (matches new tailwind config) ──────────────────────────
const C = {
  quantum:  "#4F80FF",
  neural:   "#A78BFA",
  synapse:  "#34D399",
...
const typeConfig: Record<string, { icon: any; color: string; bg: string }> = {
  layout:     { icon: Layers,         color: "text-quantum-400",  bg: "bg-quantum-400/10"  },
  typography: { icon: Eye,            color: "text-neural-400",   bg: "bg-neural-400/10"   },
  motion:     { icon: Activity,       color: "text-synapse-400",  bg: "bg-synapse-400/10"  },
  navigation: { icon: MousePointer2,  color: "text-plasma-500",   bg: "bg-plasma-500/10"   },
};
    ...
    <div className="max-w-[var(--content-max-width)] mx-auto px-8 py-8 space-y-8">
      ...
      <div className="flex items-start justify-between animate-in stagger-1">
        <div>
          ...
          <p className="text-sm text-white/38 font-sans">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-4 text-[11px] font-mono">
              <span className="flex items-center gap-1.5 text-quantum-400">
                <span className="w-3 h-0.5 rounded-full bg-quantum-400" /> Interactions
              </span>
              <span className="flex items-center gap-1.5 text-neural-400">
                <span className="w-3 h-0.5 rounded-full bg-neural-400" /> Adaptations
              </span>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={item.label} className="flex items-center gap-2.5">
                  <item.icon className="w-3.5 h-3.5 text-white/22 flex-shrink-0" />
                  <span className="text-[12px] text-white/38 font-mono flex-1">{item.label}</span>
                  <span className="text-[12px] text-white/70 font-medium capitalize">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-[11px] font-mono text-white/22">
        ...
        <div className="col-span-12 card-base p-6 animate-in stagger-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <a href="/insights" className="flex items-center gap-1.5 text-[11px] font-mono text-quantum-400 hover:text-quantum-300 transition-colors">
              <BarChart2 className="w-3.5 h-3.5" /> Full insights
            </a>
          </div>
